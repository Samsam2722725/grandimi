...
/* SonarGrid — champ de points qui répond aux clics par des ondes concentriques.
   Porté depuis la version TypeScript : ce projet est en JavaScript, donc seuls
   les types disparaissent. Le comportement, les valeurs par défaut et les noms
   de props sont ceux de l'original.

   Il lit la couleur résolue de `text-primary`, qui vaut ici `--color-coral-pulse`
   (l'orange de la marque) : le champ suit donc le thème sans réglage.

   Il se met en veille quand aucune onde n'est vivante, s'arrête hors écran et
   dans un onglet caché, et rend une grille figée sous `prefers-reduced-motion`. */
...
export function SonarGrid({
  ...
  // La boucle de rendu lit les props par cette référence : un réglage changé
  // s'applique sans la redémarrer.
  const opts = React.useRef({})
  opts.current = {
    spacing, dotRadius, baseOpacity, pingEvery, speed,
    ringWidth, amplitude, interactive, maxRings, seedPing, pingArea,
  }
  ...
  React.useEffect(() => {
    ...
    const draw = (now) => {
      ...
      // Secondes avant qu'une onde quitte le canevas.
      ...
      // Passe 1 : tous les points au repos, un seul chemin et un seul remplissage.
      const hot = []
      ...
      for (let i = 0; i < cols; i++) {
        ...
        for (let j = 0; j < rows; j++) {
          ...
          for (const r of live) {
            ...
            const k = t * t * (3 - 2 * t) * r.fade // smoothstep, atténué avec l'âge
      ...
      // Passe 2 : seuls les points sur un front d'onde ont leur propre alpha.
  ...
  // Un réglage changé pendant que la boucle dort repeint quand même.
  React.useEffect(() => {
    refreshRef.current()
